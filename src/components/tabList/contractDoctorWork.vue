<template>
  <div class="contract-doctor">
    <ul class="contract-header">
      <li>
        <span>医生姓名</span>
        <span v-if="ZLBFLAG">{{ data.doctor_name && data.doctor_name.replace(data.doctor_name.substr(0, 1), '*') }}</span>
        <span v-else>{{ data.doctor_name || '' }}</span>
      </li>
      <div v-if="data.doctor_contact">
        <li @click="phoneCall" v-if="ZLBFLAG">
          <span>电话</span>
          <span>
            <van-icon name="phone-o" color="#F88A8A" size="20px" />
            {{ data.doctor_contact ? data.doctor_contact.substr(0, 3) + '****' + data.doctor_contact.substr(8) : '' }}
          </span>
        </li>
        <a :href="'tel:' + data.doctor_contact" v-else>
          <li>
            <span>电话</span>
            <span>
              <van-icon name="phone-o" color="#F88A8A" size="20px" />
    ...
    <div class="write-record" v-if="type != 1">
      ...
      <ul class="record-table">
        ...
        <li>
          <span>医生姓名</span>
          <van-field placeholder="请输入医生姓名" v-model="doctorName" class="record-doctor" />
        </li>
      ...
      <van-button type="primary" size="small" class="submit-btn" @click="upload">提交</van-button>
    </div>
    <!-- 历史随访记录 -->
  ...
  methods: {
    ...
    phoneCall() {
      ZWJSBridge.phoneCall({
        corpId: this.data.doctor_contact
      })
        .then((res) => {
          console.log(res)
        })
    ...
    onConfirmDate(e) {
      // console.log(Date.parse(e) / 1000, 'e');
      // console.log(Date.parse(new Date()) / 1000, 'e')
