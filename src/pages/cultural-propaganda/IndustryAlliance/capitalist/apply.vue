<template>
  <div class="formCon industryCropApplyCon">
    <van-form @failed="onFailed" @submit="onSubmit">
      <van-field required input-align="left" v-model="form2.name" name="name" label="企业名称" maxlength="20" :rules="state.validator.name" placeholder="请输入企业名称" />
      <van-field
        required
        input-align="left"
        v-model="form2.social_code"
        name="social_code"
        label="统一社会信用代码"
        maxlength="20"
        :rules="state.validator.social_code"
        placeholder="请输入统一社会信用代码"
      />
      <van-field required input-align="left" v-model="form2.legal_name" name="legal_name" label="法人姓名" readonly maxlength="20" :rules="state.validator.legal_name" placeholder="请输入法人姓名" />
      <van-field required input-align="left" v-model="form2.legal_card" name="legal_card" label="法人身份证号" readonly maxlength="20" :rules="state.validator.legal_card" placeholder="请输入法人身份证号" />
      <van-field required input-align="left" v-model="form2.contact_name" name="contact_name" label="联系人姓名" maxlength="20" :rules="state.validator.contact_name" placeholder="请输入联系人姓名" />
      <van-field
        required
        input-align="left"
        v-model="form2.contact_mobile"
        name="contact_mobile"
        label="联系电话"
        maxlength="11"
        :rules="state.validator.contact_mobile"
        placeholder="请输入联系电话"
      />
      ...
      <div class="pic">
        <p class="van-field__label--required">食品生产许可证</p>
        <div class="pic-list">
          <div v-for="(item, index) in form2.pic2" :key="item.id" class="image-box">
            <van-icon class="delete" name="clear" @click="toDelete('3', index)" />
            <img :src="item.url" alt />
          </div>
          <div class="upload-btn" v-show="form2.pic2.length < 1" @click="choiceImg('3')">
            <van-icon name="plus" />
            <span>添加图片</span>
          </div>
        </div>
      ...
      <div class="bottomCon fixBottom">
        ...
        <div class="bottom">
          ...
          <van-button block class="fixButton" type="primary" :disabled="!state.checked" native-type="submit">申请加入</van-button>
        </div>
      </div>
    </van-form>
  ...
  setup() {
    ...
    const state = reactive({
      ...
      validator: {
        ...
        legal_name: [{ trigger: 'onBlur', message: '请输入', required: true }],
        legal_card: [
          {
            requied: true,
            trigger: 'onBlur',
        ...
        contact_name: [{ trigger: 'onBlur', message: '请输入', required: true }],
        social_code: [{ trigger: 'onBlur', message: '请输入', required: true }],
        contact_mobile: [
          {
            requied: true,
            trigger: 'onBlur',
      ...
      Request.getUserInfo()
        .then((res) => {
          if (res && res.code === 0) {
            ...
            form2.legal_name = shorter.real_name.replace(shorter.real_name.substr(0, 1), '*')
            form2.legal_card = shorter.zhelb_user_info.idnum.slice(0, 8) + '****' + shorter.zhelb_user_info.idnum.slice(15)
            form2.legal_nameReal = shorter.real_name
            form2.legal_cardReal = shorter.zhelb_user_info.idnum
          }
        })
        .catch((err) => {
          console.log('err', err)
        ...
        Request.IndustryErpInfo({
          ...
          .then((res) => {
            if (res && res.code == 0) {
              ...
              form2.social_code = shorter.social_code
              //   form2.legal_name = shorter.legal_name
              //   form2.legal_card = shorter.legal_card
              form2.contact_name = shorter.contact_name
              form2.contact_mobile = shorter.contact_mobile
              ...
              form2.pic = [
                {
                  url: shorter.social_img_list[0],
                  id: shorter.social_img_str
                }
              ]
              form2.pic2 = [
                {
                  url: shorter.product_img_list[0],
                  id: shorter.product_img_str
                }
              ]
            } else {
              Toast(res.msg)
    ...
    const onSubmit = (params) => {
      if (form2.pic.length && form2.pic2.length) {
        let data = {
          name: form2.name,
          social_code: form2.social_code,
          //   legal_name: form2.legal_name,
          //   legal_card: form2.legal_card,
          legal_name: form2.legal_nameReal,
          legal_card: form2.legal_cardReal,
          contact_name: form2.contact_name,
          contact_mobile: form2.contact_mobile,
          address: form2.address,
          town_id: form2.town_id,
          village_id: form2.village_id
        }
        data.social_img_str = form2.pic
          .map((item) => {
            return item.id
          })
          .join(',')
        data.product_img_str = form2.pic2
          .map((item) => {
            return item.id
          })
          .join(',')
        if (Route.query.id) {
          ...
          Request.IndustryErpEdit(data).then((res) => {
            if (res.code === 0) {
              Toast('提交成功')
              Router.replace({
          ...
          Request.IndustryErpApply(data).then((res) => {
            if (res.code === 0) {
              Toast('提交成功')
              Router.replace({
                name: 'IndustryAllianceCapSuccess'
