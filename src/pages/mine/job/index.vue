                ...
                >
                    <div class="itemCon">
                        <div class="listImgOuter">
                            <div
                                class="imgCon"
                                :style="item.image_path && item.image_path.length ? 'background-image: url(' + item.image_path[0] + ')' : 'background-image: url(https://img.hzanchu.com/acimg/a00df9ebc4b71833879c97d9f898f306.png)'"
                            ></div>
                        </div>
                        <div class="rightSide" :class="[state.ZLBFitForOld ? 'rightSideOld' : '']">
                            ...
                            <div class="firstLine">
                                <div
                                    class="title line-clamp-2"
                                    :class="[state.ZLBFitForOld ? 'titleOld' : '']"
                                >{{ item.issue_title }}</div>
                                <div
                                    class="time"
                                    :class="[state.ZLBFitForOld ? 'timeOld' : '']"
                                >{{ item.created_at.slice(0, 10) }}</div>
                            </div>
                            <div
                                class="price"
                            ...
                            <div class="lastLine">
                                <div
                                    class="itemType"
                                    :class="[state.ZLBFitForOld ? 'itemTypeOld' : '']"
                                >{{ item.category_data }}</div>
                                <div
                                    @click.stop="popDelete(item)"
                                    class="deleteBtn"
                                    :class="[state.ZLBFitForOld ? 'deleteBtnOld' : '']"
                                >
                                    <icon
                                        class="deIcon"
                                        name="https://img.hzanchu.com/acimg/32091c057bc182fb26772dd42550826a.png"
                                    />
                                    <span>删除</span>
                                </div>
                            </div>
...
import { List, PullRefresh, Icon, Divider, Dialog, Toast } from 'vant'
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      sessionStorage.removeItem('fromMyJob')
      onLoad()
    })
    const onRefresh = () => {
      state.listData = []
    ...
    const checkReason = (item) => {
      Dialog.confirm({
        ...
        .then(() => {
          sessionStorage.setItem('fromMyJob', '1')
          Router.push({
            name: 'jobPublish',
            query: {
              id: item.id,
    ...
    const popReject = (item) => {
      Dialog.confirm({
        title: '下架确认',
        message: '下架后，“劳务大厅界面”将不再展示，确认下架吗？',
      })
        .then(() => {
          // on confirm
          let data = {
