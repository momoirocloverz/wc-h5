      ...
      <pull-refresh v-model="state.refreshing" @refresh="onRefresh">
        <list v-model:loading="state.loading" class="cusVanList" :immediate-check="false" :finished="state.finished" @load="onLoad">
          <div class="firstItem" v-for="(item, index) in state.tableList1" :key="index">
            <div class="itemTop">
              <div class="itemFir">
                <div class="FirstTitleIt ellipsis" :class="[state.ZLBFitForOld ? 'FirstTitleItOld' : '']">{{ item.plant_name }}</div>
                <div class="statusCon">
              ...
              <div class="itemSec">
                <div class="titleItem" :class="[state.ZLBFitForOld ? 'titleItemOld' : '']">预约时间：</div>
                <div class="statusCon static1" :class="[state.ZLBFitForOld ? 'static1Old' : '']">{{ item.book_date }}{{ item.book_time }}</div>
              </div>
              <div class="itemThird">
            ...
            <div class="itemBottom">
              ...
              <div class="opearateBtn ml16" @click="toEdit(item)" v-if="item.status == 0">修改资料</div>
            </div>
          </div>
        </list>
...
export default {
  ...
  setup() {
    ...
    const initAction = () => {
      state.loading = true
      Request.IndustryPlantApplyList({
        page_size: 10,
        page: state.page
      })
        .then((res) => {
    ...
    const toEdit = (item) => {
      sessionStorage.setItem('preOrderInfo1', JSON.stringify(item))
      Router.push({
