        ...
        <list v-model:loading="state.loading" class="cusVanList" :immediate-check="false" :finished="state.finished" @load="onLoad">
          <div class="firstItem" v-for="(item, index) in state.tableList1" :key="index">
            <div class="itemTop" @click="toDetail(item)">
              <div class="itemFir">
                <div class="titleItem" :class="[state.ZLBFitForOld ? 'titleItemOld' : '']">委托加工量：</div>
                <div class="statusCon">
                  <div class="static1" :class="[state.ZLBFitForOld ? 'static1Old' : '']">{{ item.weight }}斤</div>
              ...
              <div class="itemSec">
                <div class="titleItem" :class="[state.ZLBFitForOld ? 'titleItemOld' : '']">加工费用：</div>
                <div class="statusCon static1" :class="[state.ZLBFitForOld ? 'static1Old' : '']">{{ item.price }}元</div>
              </div>
              <div class="itemThird">
                <div class="titleItem" :class="[state.ZLBFitForOld ? 'titleItemOld' : '']">预约人：</div>
                <div class="statusCon static1" :class="[state.ZLBFitForOld ? 'static1Old' : '']">{{ item.name ? item.name.replace(item.name.substr(0, 1), '*') : '-' }}</div>
              </div>
              <div class="itemFourth">
                <div class="titleItem" :class="[state.ZLBFitForOld ? 'titleItemOld' : '']">联系方式：</div>
                <div class="statusCon static1" :class="[state.ZLBFitForOld ? 'static1Old' : '']">{{ item.mobile ? item.mobile.replace(item.mobile.substr(3, 4), '****') : '' }}</div>
              </div>
              <div class="itemFifth">
            ...
            <div class="itemBottom" v-if="item.status == 0">
              <div class="opearateBtn ml16" @click="toEdit(item)">修改资料</div>
            </div>
          </div>
        </list>
...
export default {
  ...
  setup() {
    ...
    const initAction = () => {
      state.loading = true
      Request.IndustryplantProcessApplyList({
        page_size: 10,
        page: state.page,
        type: 1
      })
