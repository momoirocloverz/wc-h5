<template>
  <div class="outer">
    <div class="steps">
      <div>
        <img src="../../assets/imgs/step1.png" alt v-show="active === '1'" />
        <img src="../../assets/imgs/step_ok.png" alt v-show="active === '2'" />
        <p :style="{ color: '#FF814F' }">基本信息填写</p>
      </div>
      <span></span>
      <div>
        <img src="../../assets/imgs/step2.png" alt v-show="active === '1'" />
        <img src="../../assets/imgs/step2_s.png" alt v-show="active === '2'" />
    ...
    <div class="center" v-show="active === '1'">
      <div class="item">
        <span>户主姓名</span>
        <input type="text" placeholder="请填写户主姓名" v-model="info.name" @blur="onBlur" />
      </div>
      <!-- <div class="fakeBorder"></div> -->
      <div class="item">
        <span>户主身份证号</span>
        <input type="text" placeholder="请填写户主身份证号" v-model="info.cardID" @blur="onBlur" />
      </div>
      <!-- <div class="fakeBorder"></div> -->
      <div class="item">
        <span>户主电话</span>
        <input type="tel" placeholder="请填写户主电话" v-model="info.phone" />
      </div>
      <!-- <div class="fakeBorder"></div> -->
      <div class="item" @click="selectType">
        <span>类型</span>
        <span :style="{ color: info.type ? '#333' : '#b8b8b8' }">
          {{ info.type || '请选择类型' }}
        </span>
        <van-icon name="arrow" />
      </div>
      ...
      <div class="item">
        <span>子女姓名</span>
        <input type="text" placeholder="请填写子女姓名" v-model="info.childrenName" />
      </div>
      <!-- <div class="fakeBorder"></div> -->
      <div class="item">
        <span>子女身份证号</span>
        <input type="text" placeholder="请填写子女身份证号" v-model="info.childrenCardID" />
      </div>
      <!-- <div class="fakeBorder"></div> -->
      <div class="item" @click="selectRelative">
        <span>关系</span>
        <span :style="{ color: info.relative ? '#333' : '#b8b8b8' }">
          {{ info.relative || '请选择与户主的关系' }}
        </span>
        <van-icon name="arrow" />
      </div>
      <!-- <div class="fakeBorder"></div> -->
      <div class="item">
        <span>子女就读院校</span>
        <input type="text" placeholder="请填写子女就读院校" v-model="info.school" />
      </div>
      <!-- <div class=" spBorder"></div> -->
      <!-- <div class="item"
           @click="selectSchool">
        <span>院校类型</span>
        <span :style="{color: info.schoolType ? '#333' : '#b8b8b8'}">{{info.schoolType || '请选择院校类型'}}</span>
        <van-icon name="arrow" />
            </div>-->
      <!-- <div class="fakeBorder"></div> -->
      <div class="item" @click="selectTown">
        <span>所属乡镇</span>
        <span :style="{ color: info.town ? '#333' : '#b8b8b8' }">
          {{ info.town || '请选择所属乡镇' }}
        </span>
        <van-icon name="arrow" />
      </div>
      ...
      <div class="item" @click="selectVillage">
        <span>所属村</span>
        <span :style="{ color: info.village ? '#333' : '#b8b8b8' }">
          {{ info.village || '请选择所属村' }}
        </span>
        <van-icon name="arrow" />
      </div>
      <!-- <div class="item">
        <span>申请日期</span>
        <span :style="{color: info.time ? '#333' : '#b8b8b8'}">{{info.time}}</span>
        <img src="https://img.hzanchu.com/acimg/41eff3906c031196192c8dff69f9f44f.png" alt="">
            </div>-->
    </div>
  ...
  setup() {
    ...
    const getInfo = () => {
      Request.info({
        ...
        if (res.code === 0) {
          info.name = res.data.head_name
          info.cardID = res.data.head_card
          info.phone = res.data.head_contact
          info.type = res.data.type_name
          info.typeId = res.data.type
          info.childrenName = res.data.children_name
          info.childrenCardID = res.data.children_card
          info.relative = res.data.relation
          info.school = res.data.college_name
          // info.schoolType = res.data.college_type;
          info.town = res.data.town_name
          info.townId = res.data.town_id
          getVillageList()
          info.village = res.data.village_name
          info.villageId = res.data.village_id
          info.image1.url = res.data.hkb_hz_img
          info.image2.url = res.data.hkb_zn_img
          info.image3.url = res.data.hz_card_zm_img
          info.image4.url = res.data.hz_card_fm_img
          info.image5.url = res.data.zv_card_zm_img
          info.image6.url = res.data.zv_card_fm_img
          info.image7.url = res.data.zv_sb_zm_img
          info.image8.url = res.data.zv_sb_fm_img
          info.image9.url = res.data.zv_jdzm_img
          info.image10.url = res.data.smcn_img
        }
      })
    }
    ...
    const toNext = () => {
      if (!info.name || !info.phone || !info.cardID || !info.childrenName || !info.childrenCardID || !info.type || !info.relative || !info.school || !info.townId || !info.villageId) {
